...
function giffer() {
  ...
  function record() {
    const count    = parseFloat(cursor.get('count')) || 300
        , rate     = parseFloat(cursor.get('rate')) || 60
        , ext      = '.jpg'
        , buffer   = cursor.get('buffer') || 'composite'
    ...
    function capture() {
      let t = now()
        ...
        , filename = `data/giffer/frames/${buffer}.${startTime}.${index}${ext}`

      lastFrameTime = t

      loopin.patch( {
        enabled: true,
        text: `giffer - frame ${index}`
      }, 'osd/')
    ...
    function finalize() {
      const ffInput = loopin.filesAbsolute(`data/giffer/frames/${buffer}.${startTime}.%04d${ext}`)
          , ffOutput = loopin.filesAbsolute(`data/giffer/${buffer}.${startTime}.mp4`)
          , framesGlob = loopin.filesAbsolute(`data/giffer/frames/${buffer}.${startTime}.????${ext}`)
          , rate = 60
          , ffCommand = 'ffmpeg -framerate {{ rate }} -i {{ ffInput }} {{ ffOutput }}'

      loopin.patch( {
        enabled: true,
        text: `giffer - finalizing`
      }, 'osd/')
